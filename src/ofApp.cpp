...
void agent::draw(){
  ...
  p1.rotate(ofRandom(angle/2,angle));	// point in direction of movement
  p2.rotate(ofRandom(angle/2,angle));
  p3.rotate(ofRandom(angle/2,angle));
...
void ofApp::setup(){
    ...
    ofSetFrameRate(30); // Limit the speed of our program to 60 frames per second
    ofBackground(0);
...
void ofApp::mousePressed(int x, int y, int button){
    Agents[curAgent].start(ofRandom(x,x*6), ofRandom(y,y*6));
    curAgent++;
